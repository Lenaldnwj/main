...
[NOTE]
The diagram label numbers in subsequent sections might be inconsistent as they are extracts from various parts of the user guide and
developer guide.
...
As we increased the number of attributes for the person being added into the address book, we realised that it was getting very cumbersome for the user to
...
be optional inputs/fields for the user.
